...
protected:
   ...
   void print(Link a, std::string prefijo, bool esDerecho) const {
      if (a != nullptr) {
         // Imprimir el subárbol derecho (queda arriba en la consola)
         print(a->dr, prefijo + (esDerecho ? "    " : "│   "), true);
         
         // Imprimir el nodo actual
         std::cout << prefijo;
         if (prefijo == "") std::cout << "─── ";
         else std::cout << (esDerecho ? "┌── " : "└── ");
         std::cout << a->elem << "\n";
         
         // Imprimir el subárbol izquierdo (queda abajo en la consola)
         print(a->iz, prefijo + (esDerecho ? "│   " : "    "), false);
      }
   }
